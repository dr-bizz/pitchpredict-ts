...
export class LeaderboardService {
  ...
  async rows(): Promise<LeaderboardRow[]> {
    ...
    const withBonus = aggregated.map((u) => ({
      ...
      totalPoints:
        u.totalPoints +
        (bonusUserIds.has(u.id) ? ScoringService.CHAMPION_BONUS : 0),
    }));
  ...
  private async aggregatedUsers(): Promise<AggregatedUser[]> {
    ...
    const rows = await this.db
      .select({
        ...
        exactCount: countWhere(ScoringService.EXACT),
        diffCount: countWhere(ScoringService.DIFFERENCE),
        tendencyCount: countWhere(ScoringService.TENDENCY),
      })
      .from(schema.users)
      .leftJoin(
        schema.predictions,
  ...
  private async championBonusUserIds(): Promise<Set<number>> {
    const championId = await this.scoring.championTeamId();
    if (championId === null) {
      return new Set();
    }
    const rows = await this.db
