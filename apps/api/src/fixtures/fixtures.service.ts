...
export class FixturesService {
  ...
  async list(
    ...
    const resolvedStage = this.resolveStage(stage);

    const rows: FixtureWithRelations[] =
      resolvedStage === 'upcoming'
        ? await this.db.query.fixtures.findMany({
            where: and(
    ...
    const fixtures = rows.map((row) => this.withLocked(row, now));
    const groups =
      resolvedStage === 'upcoming'
        ? this.groupByDate(fixtures)
        : resolvedStage === 'group'
        ? this.groupByGroupName(fixtures)
        : [{ label: '', fixtures }];

    const predictionsByFixtureId = await this.callerPredictions(
      userId,
      rows.map((r) => r.id)
    );
  ...
  private resolveStage(stage: string | undefined): string {
    const tabs = [
      'upcoming',
      'group',
      'r32',
  ...
  private groupByDate(fixtures: FixtureWithTeams[]): FixtureGroup[] {
   ...
   */
  private groupByGroupName(fixtures: FixtureWithTeams[]): FixtureGroup[] {
    const groups: FixtureGroup[] = [];
    for (const fixture of fixtures) {
  ...
  private async callerPredictions(
    userId: number,
    fixtureIds: number[]
    ...
    const rows = await this.db.query.predictions.findMany({
      where: and(
